...
public class MarketTest {
    ...
    void testBuyAndSell() {
        ...
        //Now let's try to exchange one commodity on another with mix of trading Currencies 
        market2.buy(trader, com2, 1); // I want to get 1 silver from this market I need to exchange silver to the currency. That is used on that market
        // so Ill get 2 zloty for srebro, then get 1 of main currency, then 0.5 yang. So I must pay 0.55 of yangs. 

        assertEquals((float)10.45, investmentBudget.get("yang"));
        assertEquals((float)11.0, investmentBudget.get("srebro"));

        //Everything seems fine!! Now lets test situation when I don't have trading currency on that market.
        investmentBudget.remove("yang");
        ...
        //So I have 22 zloty to spend So I can get 20 zloty for that since provision then will be 2 zloty
        ...
        //I want to buy 0,8545454545454545 gold. Having 3 yangs and 6.8 zlotys and the currency is yang.
        //So for 3 yangs I get 3 yangs and for 6.8 zloty I get 1.7 yangs so I have 4.7 yangs. So by subtracting operation cost
        //I get 4,272727272727273 of yangs available.
        market2.buy(trader, com1, (float)0.8545454545454545);

        assertEquals((float)85, Math.round(investmentBudget.get("zloto")*100)); //0.85 since floating point precision may mess up now
